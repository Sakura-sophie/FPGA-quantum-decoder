...
The valid handshake pulses once per pixel, but the camera's valid signal is asynchronous to the FPGA's internal clock and can stay high for several clock cycles per pixel, latching the same pixel multiple times before valid dropped again. To avoid this, valid is passed through two flip-flops ('valid_1', 'valid_2') and then combined as valid_1 AND NOT valid_2. This produces a new signal, 'valid_phase', that only pulses high for exactly one clock cycle, right on the rising edge of valid. That single pulse is what triggers the BRAM write, guaranteeing each pixel is captured exactly once. ![valid_phase](docs/valid_phase.png)
...
- **Main body FSM**
Sequences the pipeline through its stages - image capture, detection/counting, target generation, rearrangement calculation, and DAC output, with the 'Q1'-'Q4' flags exposing which stages have complete.

- **Detection & counting**
Within the first state, FINDING_1, each pixel inside an ROI is compared against 'hist_data_h' and 'hist_data_l'. Above the high threshold registers an atom hit; between the two thresholds flags an ambiguous case without counting it as an atom; below the low threshold is background.
Hits are OR-accumulated across all pixels in a site's ROI window (from 'roi_first_pixel' to 'roi_last_pixel'), so a single bright pixel anywhere in the ROI is enough to classify that site as occupied. This makes detection tolerant to the atom not falling exactly on the ROI's centre pixel. The result is stored as 'roi_results', a grid_size × grid_size array of bits (one per lattice site), and the total atom count ('atom_counter') is incremented once per site as the frame scan completes.
