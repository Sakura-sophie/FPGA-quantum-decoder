...
begin
  ...
  --  end process;
    ...
    finding_ones : process(clock, reset)
        ...
        variable best_dist : integer := integer'high;
        variable best_r, best_c : integer range 0 to grid_size-1;
        variable cur_dist : integer;
        variable donor_found : boolean := false;
            ...
            delay_s <= 0;
 	          ...
 	          --if clk_en='1' then
                case STATE is
                    ...
                    when SEARCH_DONOR =>
                        best_dist := integer'high;
                        donor_found :=false;
                        
                        DONOR_R: for r_2 in 0 to (grid_size -1) loop
                            DONOR_C: for c_2 in 0 to (grid_size -1) loop
                                if var_results(r_2)(c_2) = '1' and target_grid(r_2)(c_2) = '0' then
                                    cur_dist := abs(target_r - r_2) + abs(target_c - c_2);
                        ...
                        if donor_found then
                            var_results(best_r)(best_c) := '0';
                            var_fifo(index).x_co     := std_logic_vector(TO_UNSIGNED(best_r, 8));
                            var_fifo(index).y_co     := std_logic_vector(TO_UNSIGNED(best_c, 8));
                            var_fifo(index).x_move_v := TO_SIGNED(target_r - best_r, 8);
